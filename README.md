...
构建 Windows 安装包后，安装向导会分别询问软件安装位置、本地数据储存位置和是否创建桌面快捷方式。开发或迁移到新电脑时先执行 `npm ci`，再按构建命令生成安装包。
...
- `build/`：Windows 安装器的自定义安装页面脚本
- `test/`：规则与接口配置测试
- `package.json`、`package-lock.json`：需要继续开发或重新构建时使用

`release/` 和 `release-installer/` 仅为本机构建产物，不属于源代码；需要运行或发布时先执行 `npm ci`，再按目标平台执行构建命令。
